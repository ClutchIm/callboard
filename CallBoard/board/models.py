...
class User(AbstractUser):
    """
        Модель дополняющая модель стандартного User'а, содержит поля:

        email - переопределенное поля для добавления уникального e-mail адреса
        is_verified - булево поле для определения верифицирован ли пользователь
        email_otp - поле для хранения одноразового кода, после верификации становится пустым
        user_response - булево поле для определения хочет ли пользователь получать
    уведомления об откликах оставивших под его постами
        news_subscription - булево поле для определения будет ли получать пользователь
    новостную рассылку
    """
